...
/**
   @brief read coefficients of anaysis and synthesis prototypes.
...
#define _DEBUG_
#ifdef _DEBUG_
  fprintf( stderr, "coeffcient vector for an analysis filter\n");
  for (int i=0; i<filterLen ;i++ ){
    fprintf( stderr, "%e ", gsl_vector_get( coeffVA, i ) );
  }
  fprintf( stderr, "\n");
  fprintf( stderr, "coeffcient vector for a synthesis filter\n");
  for (int i=0; i<filterLen ;i++ ){
    fprintf( stderr, "%e ", gsl_vector_get( coeffVS, i ) );
  }
  fprintf( stderr, "\n");
#endif
  coeffSet[0] = coeffVA;
...
gsl_vector *calcDelaysPolar2( float azimuth, float elevation, const char *micPosFn )
{
  gsl_vector *delays;
 ...
 */
bool doBeamforming( const char *audioList, const char *coeffFile, const gsl_vector *delays, double sampleRate, unsigned fftLen, int pf, double alpha, unsigned D, unsigned m, unsigned r, const char *outputFile )
{
  FILE *fp;
  ...
  SubbandGSCPtr beamformerP = new SubbandGSC( fftLen, false, pf, alpha );
  OverSampledDFTSynthesisBankPtr synthesisFBP = new OverSampledDFTSynthesisBank((VectorComplexFeatureStreamPtr&)beamformerP, g_fb, fftLen, m, r );
  //OverSampledDFTSynthesisBankPtr synthesisFBP;
  ...
  while(1){
    ...
    //synthesisFBP = new OverSampledDFTSynthesisBank((VectorComplexFeatureStreamPtr&)analysisFBP, g_fb, fftLen, m, r );
    //break;
  }  
  fclose(fp);
  beamformerP->calcGSCWeights( sampleRate, delays );
...
#ifdef _DEBUG_
  fprintf(stderr,"output wave data\n");
#endif
  SF_INFO sfinfo;
...
#ifdef _DEBUG_
  fprintf(stderr,"\n");
  list<SampleFeaturePtr>::iterator itr1, itr1E;
  list<OverSampledDFTAnalysisBankPtr>::iterator itr2, itr2E;
  itr1  = sampleFeaturePL.begin();
  itr1E = sampleFeaturePL.end();
  itr2  = analysisFBPL.begin();
  itr2E = analysisFBPL.end();
  for(;itr1!=itr1E;itr1++,itr2++){
    SampleFeaturePtr hook1 = *itr1;
    OverSampledDFTAnalysisBankPtr hook2 = *itr2;
    //delete hook1;
    //delete hook2;
  }
#endif
...
int main( int argc, char **argv )
{
  ...
  std::string audioList = "./testL";
  std::string micPosFile  = "./position";
  std::string coeffFile  = "./M256-m4-r1";
  std::string outputFile  = "./beamformed.wav";
  ...
  delays = calcDelaysPolar2( azimuth, elevation, micPosFile.c_str() );
  doBeamforming( audioList.c_str(), coeffFile.c_str(), delays, sampleRate, M, pf, alpha, D, m, r, outputFile.c_str() );
