#
#                        Beamforming Toolkit 2.0
#                               (BTK 2.0)
#
#   Module:  btk20.py_source_localizer
#   Purpose: Source localizer
...
class LinearArraySRPDOAEstimator:
    ...
    def calcSteeringMatrix( self, baseMicX = -1 ):
        ...
        phaseShifts = numpy.zeros( self._nChan, numpy.float )
        J = (0+1j)
        Delta_f = J * 2.0 * numpy.pi * self._sampleRate / float(self._fftLen)
        ...
        self._steeringMatrix = numpy.zeros( (self._fftLen2+1, self._nDOA, self._nChan) , numpy.complex )
        norm = self._nChan
        for fbinX in range(self._fftLen2+1):
            for dirX in range(self._nDOA):
                sintheta = self._minDOA + dirX * self._deltaSin
                for chanX in range(self._nChan):
                    self._steeringMatrix[fbinX][dirX][chanX] = numpy.exp( phaseShifts[chanX] * fbinX * sintheta ) / norm
    ...
    def calcSRP( self, X, lowerFBin = 1, upperFBin = -1 ):
        ...
        self._Y2 = numpy.zeros( self._nDOA, numpy.float)
        self._maxY2    = 0.0
        self._argMaxY2 = 0
        for dirX in range(self._nDOA):
            for fbinX in range(lowerFBin,upperFBin):
                Y = abs( numpy.matrixmultiply( numpy.conjugate(self._steeringMatrix[fbinX][dirX]), X[fbinX] ) )
                self._Y2[dirX] += Y * Y
